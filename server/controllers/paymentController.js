    ...
    if (packageId) {
      ...
      if (pkg) {
        ...
        if (payment && payment.selectedCourseIds && payment.selectedCourseIds.length > 0 && payment.selectedCourseIds !== 'undefined' && payment.selectedCourseIds[0] !== '') {
          baseCoursesToGrant = Array.isArray(payment.selectedCourseIds) ? payment.selectedCourseIds : [payment.selectedCourseIds];
        } else {
          baseCoursesToGrant = (pkg.courseIds && pkg.courseIds.length > 0) ? pkg.courseIds : ['course-class-1','course-class-2','course-class-3','course-class-4','course-class-5','course-class-6','course-class-7','course-class-8'];
        }
        
        let coursesToGrant = baseCoursesToGrant.map(id => `${id}_${pkg._id}`);
